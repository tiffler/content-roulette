...
const generators = {
  ...
  funJobTitle() {
    ...
    if (r < 0.33)
      return (
        pick(DATA.jobTitleFunnyModifiers) +
        " " +
        pick(DATA.jobTitleFunnyNouns) +
        " " +
        pick(DATA.jobTitleFunnyRoles)
      );
    if (r < 0.66)
      return (
  ...
  jobTitle() {
    ...
    if (!hasPrefix && Math.random() < 0.25)
      parts.push(pick(DATA.jobTitleSuffixes));
    return parts.join(" ");
  },
  ...
  personalEmails() {
    ...
    const suf =
      numD > 0
        ? ("" + Math.floor(Math.random() * Math.pow(10, numD)))
            .slice(0, numD)
            .padStart(numD, "0")
        : "";
  ...
  username() {
    ...
    const num =
      Math.random() < 0.5
        ? (Math.floor(Math.random() * 9000) + 10).toString()
        : "";
  ...
  apps() {
    ...
    for (let i = 0; i < wc - 1; i++) {
      words.push(
        i === 0 ? pick(DATA.appNameAdjectives) : pick(DATA.appNameModifiers),
      );
    }
  ...
  mac() {
    ...
    switch (Math.floor(Math.random() * 6)) {
      ...
      case 1:
        return `${device} ${model} ${size} - ${cpu}`;
      case 2:
        return `${device} ${model} ${size} - ${cpu} - (${year})`;
      case 3:
        return `${device} ${model} ${size} ${cpu} (${year} - ${ram} RAM - ${storage} SSD)`;
      case 4:
        return `${device} ${model} ${size} - ${cpu} - ${year} - ${ram} RAM - ${storage} SSD`;
      case 5:
        return `${device} ${model} ${size} - ${cpu} - ${year} - ${ram} RAM - ${storage} SSD - ${color}`;
      default:
        return `${device} ${model} ${size}`;
    }
  },
...
function generateBulk() {
  ...
  const count = Math.max(
    1,
    parseInt(document.getElementById("bulk-count").value, 10) || 10,
  );
...
function copyBulk() {
  ...
  if (!txt || txt === "Select a type and count, then click Generate Bulk")
    return;
